...
control 'cis-dil-benchmark-6.1.11' do
  ...
  describe command("df --local -P | awk '{ if (NR!=1) print $6 }' | xargs -I '{}' find '{}' -xdev -nouser | grep -v systemd | grep -v snap") do
    its(:stdout) { should eq '' }
  end
end
...
control 'cis-dil-benchmark-6.1.12' do
  ...
  describe command("df --local -P | awk '{ if (NR!=1) print $6 }' | xargs -I '{}' find '{}' -xdev -nogroup | grep -v systemd | grep -v snap") do
    its(:stdout) { should eq '' }
  end
end
